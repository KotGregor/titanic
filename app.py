...
MODEL_PATH = os.path.join(os.path.dirname(__file__), "data", "model_weights.mw")

def ensure_model_exists():
    if not os.path.exists(MODEL_PATH):
        st.warning("Модель не найдена. Обучаем ...")
        try:
            df = open_data()
            X_df, y_df = preprocess_data(df)
            fit_and_save_model(X_df, y_df, path=MODEL_PATH)
